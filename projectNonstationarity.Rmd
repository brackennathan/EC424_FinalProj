...
pacman::p_load(here, data.table, tseries, tidymodels, janitor, stringr, tframe)
```
...
adf.test(food_cts$UnitedKingdom$`Daily caloric supply (OWID based on UN FAO & historical sources)`, k = 
...
# function that accepts some window and runs the dicke fuller test over the window


# function that accepts some vector and returns a tibble logging the window start and end
e_intv = function(series) {
  
df_i = function(i){
adf.test(window_i, k = 3)
}
# find the length of the vector
  ...
  tibble(
    window_n = i,
    window_start = window_i$start,
    window_end = window_i$end,
    nonstat = df_i$p_value
  )
}
